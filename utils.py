...
def generate_masks_color(mask_path):
    if 'hammer' in mask_path:
        mask = scio.loadmat(mask_path + '/hammer_cacti_bayer.mat')
    else:
        mask = scio.loadmat(mask_path + '/mid_color_mask.mat')
    mask = mask['mask']
    mask = np.transpose(mask, [2, 0, 1])
    mask_s = np.sum(mask, axis=0)
    index = np.where(mask_s == 0)
