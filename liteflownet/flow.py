...
def backwarp(tenInput, tenFlow):
    ...
    return torch.nn.functional.grid_sample(input=tenInput,
                                           grid=(backwarp_tenGrid[str(tenFlow.shape)].to(tenFlow.device) + tenFlow).permute(0, 2, 3, 1),
...
def estimate(tenOne, tenTwo, netNetwork):
    # global netNetwork
